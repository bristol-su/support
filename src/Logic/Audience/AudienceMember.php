...
class AudienceMember implements Arrayable, Jsonable
{
    ...
    {
        if($this->groups == null) {
            $this->groups = collect(app(GroupRepository::class)->allFromStudentControlID($this->user->id));
        }
        return $this->groups;
    }
    ...
    {
        if($this->roles == null) {
            $this->roles = collect(app(RoleRepository::class)->allFromStudentControlID($this->user->id));
        }
        return $this->roles;
    }
    ...
    public function filterForLogic(Logic $logic)
    {
        ...
        $this->roles = $this->roles()->filter(function(Role $role) use ($logic) {
            try {
                return LogicTester::evaluate($logic, $this->user, app(GroupRepository::class)->getById($role->group_id), $role);
