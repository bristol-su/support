...
namespace BristolSU\Support\Tests\Unit\Logic\Specification;
...
class InFilterAudienceSpecificationTest extends TestCase
{
    ...
    public function it_gets_the_filter_by_alias(){
        ...
        $specification = new InFilterAudienceSpecification($item, $filterInstance->reveal(), $filterRepository->reveal());
        $specification->isSatisfied();
    }

    /** @test */
    public function it_returns_true_if_the_item_id_is_in_the_filter_audience_id(){
        $filterInstance = $this->prophesize(FilterInstance::class);
        $filterInstance->alias()->shouldBeCalled()->willReturn('alias1');
        $filterInstance->settings()->shouldBeCalled()->willReturn([]);
    ...
    /** @test */
    public function it_returns_false_if_the_item_id_is_not_in_the_filter_audience_id(){
        $filterInstance = $this->prophesize(FilterInstance::class);
        $filterInstance->alias()->shouldBeCalled()->willReturn('alias1');
        $filterInstance->settings()->shouldBeCalled()->willReturn([]);
